...
- [ ] spawn-new-sticky-beside-parent positioning — implementable now with the merged `screen` injection but deferred per user preference; see `_newSticky` for the integration point.
- [ ] lock Y-axis resize while folded — wrapper has no max-height hook; snap-back from plugin side (whether via `heightChanged` debounce or `SettingsData.desktopWidgetInstancesChanged` Connections) doesn't reliably catch the drag-release on Niri. Needs upstream wrapper change to expose either a max-height or an `interactive`/`isInteracting` signal so the plugin can detect drag-end deterministically.
